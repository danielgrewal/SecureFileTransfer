...
def get_user(username: str):
    ...
    params = (username,)
    result = db.query(
        "select username, password_hash from users where users.username = %s;", params)
...
def create_token(data: dict):
    ...
    encoded_jwt = jwt.encode(token_data, SIGNATURE_KEY,
...
def extract_username(token: str) -> str:
    try:
        token_content = jwt.decode(
            token, SIGNATURE_KEY, algorithms=[SIGNATURE_ALGORITHM])
...
async def start_session(request: Request, token: str = Depends(oauth2_scheme)):
    ...
    params = (username_initiator, username_responder, role_initiator,
              address_initiator, port_initiator, aes_key)
